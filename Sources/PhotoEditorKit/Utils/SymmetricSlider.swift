...
public struct SymmetricSlider: View {
    ...
    public var body: some View {
        GeometryReader { geometry in
            ...
            // Calculate positions
            let zeroX = xPosition(for: 0, width: width, thumbSize: thumbSize)
            ...
            ZStack(alignment: .leading) {
                ...
                let startX = min(zeroX, currentX)
                let segmentWidth = abs(currentX - zeroX)

                RoundedRectangle(cornerRadius: trackHeight / 2)
                    .fill(value >= 0 ? Color.accentColor : Color.blue)
    ...
    private func xPosition(for v: Double, width: CGFloat, thumbSize: CGFloat) -> CGFloat {
        ...
        let percent = (clampedValue - minVal) / range
...
#Preview {
    ...
    SymmetricSlider(value: $value, bounds: -100 ... 100, step: 1)
}
